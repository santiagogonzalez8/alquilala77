...
/**
 * Leer sin auth — para páginas públicas (home, detalle de propiedad)
 * Usa runQuery con apiKey para filtrar por campo
 */
export async function firestoreGetPublic(collection, filters = [], limitCount = 6) {
  ...
  if (filters.length > 0) {
    query.structuredQuery.where = filters.length === 1
      ? {
          fieldFilter: {
            field: { fieldPath: filters[0].field },
            op: filters[0].op,
...
/**
 * Leer un documento sin auth por ID — para página de detalle pública
 */
export async function firestoreGetPublicById(collection, id) {
  ...
  if (!res.ok) throw new Error(`No se encontró el documento: ${res.status}`);
  const doc = await res.json();
  return docToObject(doc);
}