 ...
 * Soporta filtros simples: [{ field, op, value }]
 * op: 'EQUAL' | 'GREATER_THAN' | 'LESS_THAN' etc.
 */
export async function firestoreGetAll(collection, filters = []) {
  const token = await getToken();

  if (filters.length > 0) {
    // Usar runQuery para filtros
  ...
  // Sin filtros: listar colección completa
...
/**
 * Crear documento (POST — ID autogenerado)
 */
...
/**
 * Crear/sobreescribir documento con ID específico (PUT)
 */
...
/**
 * Actualizar campos específicos de un documento (PATCH con updateMask)
 */
...
/**
 * Leer sin auth (para colecciones públicas — home)
 */
export async function firestoreGetPublic(collection, filters = []) {
  const apiKey = firebaseConfig.apiKey;

  if (filters.length > 0) {
    const url = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/${DATABASE}/documents/${collection}:runQuery?key=${apiKey}`;
    const body = {
      structuredQuery: {
        from: [{ collectionId: collection }],
        where: {
  ...
  const res = await fetch(
    `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/${DATABASE}/documents/${collection}?key=${apiKey}`
  );
  if (!res.ok) return [];
  const data = await res.json();
  return (data.documents || []).map(docToObject);
